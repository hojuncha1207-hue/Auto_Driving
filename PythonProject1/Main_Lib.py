...
class Traffic_light_Detector:
  ...
  def color_detection(self, img, color_id):
      """특정 색상 영역을 추출하고 노이즈를 제거한 마스크를 반환"""
      ...
      if color_id == RED:
          ...
          mask = cv2.inRange(hsv, np.array([45, 100, 100]), np.array([95, 255, 255]))
      else:
  ...
  def object_detection(self, img):
      ...
      for cid in [RED, GREEN]:
          extracted, mask = self.color_detection(img, cid)
          if mask is None:
              continue


          # HoughCircles를 이용한 원형 검출
      ...
      return detected_color, debug_mask


  def get_stop_line(self, frame):
      """화면 하단에서 정지선(흰색 가로선)의 Y 좌표를 반환"""
      if frame is None:
          return None
      ...
      if lines is not None:
          for line in lines:
              x1, y1, x2, y2 = line[0]
              if abs(y2 - y1) < 10 and abs(x2 - x1) > 40:
                  actual_y = (y1 + y2) // 2 + roi_y_start
                  if stop_y is None or actual_y > stop_y:
                      stop_y = actual_y
      return stop_y
...
class LidarObjectDetector:
  def __init__(self, port='COM5'):
      ...
      try:
          # timeout을 줄여 데이터가 없을 때 빠르게 넘어가도록 설정
          self.lidar = RPLidar(port, timeout=0.5)
          self.lidar.connect()
          self.lidar.start_motor()
          # 버퍼 크기를 넉넉하게 잡되, 내부적으로 계속 비워줄 예정입니다.
          self.scan_generator = self.lidar.iter_scans(max_buf_meas=3000)
          self.is_connected = True
          print(f" LiDAR connected on {port}")
      except Exception as e:
          print(f" LiDAR Connection Error: {e}")

  def get_obstacle_status(self, min_angle=-30, max_angle=30, max_dist=400):
      if not self.is_connected:
          return False, 0, 0.0

      try:
          # [핵심] 버퍼 비우기 (Cleaning Buffer)
          # 쌓여있는 낡은 스캔 데이터들을 빠르게 넘기고 가장 마지막(최신) 것만 사용
          scan = next(self.scan_generator)

          # 루프를 돌면서 현재 시점에 쌓인 모든 스캔을 소비 (데이터 밀림 방지)
          # 파이썬 속도가 라이다보다 느릴 경우를 대비한 안전장치
          for _ in range(2):  # 최대 2개까지 스킵하며 최신성 유지
              try:
                  scan = next(self.scan_generator)
              except:
                  break
          ...
          for (_, angle, distance) in scan:
              ...
              if min_angle <= adj_angle <= max_angle:
                  # 15cm ~ 설정 거리 사이의 물체 감지
                  if 150 <= distance <= max_dist:
                      obstacle_count += 1
                      total_dist += distance
